#!/usr/bin/env bash
# Fine-tune FLUX.2-klein ImageWAM on the EmbodyX/old_new_scan_lerobot (scan barcode) real dataset.
# Mirrors scripts/flux2/run_train_flux2_klein_imagewam.sh but for the SCAN LeRobot v2.1 dataset.
#
# Required env (set in .env.local or the shell):
#   SCAN_DATA_ROOT   path to the LeRobot root (the dir containing data/ meta/ videos/)
...
imagewam_require_env SCAN_DATA_ROOT
...
case "${FLUX2_VARIANT}" in
  4b)
    ...
    TASK_NAME="scan_flux2_klein_4b_base_imagewam"
  ...
  9b)
    ...
    TASK_NAME="scan_flux2_klein_9b_base_imagewam"   # create this task analogously if you use 9B
...
ACTION_DIM=14
QWEN_CACHE_DIR="${QWEN_CACHE_DIR:-${SCAN_DATA_ROOT}/flux2_qwen3_cache_${FLUX2_VARIANT}}"
DATASET_OVERRIDES=(
  "data.train.dataset_dirs=[${SCAN_DATA_ROOT}]"
  "data.val.dataset_dirs=[${SCAN_DATA_ROOT}]"
...
ACTION_INIT="${ACTION_INIT:-checkpoints/action_dit_flux2_${FLUX2_VARIANT}_scan_init.pt}"
export PYTHONPATH="${REPO_ROOT}/src:${FLUX2_SRC}/src:${FLUX2_SRC}${PYTHONPATH:+:${PYTHONPATH}}"

imagewam_print_config TASK_NAME FLUX2_VARIANT SCAN_DATA_ROOT FLUX2_SRC FLUX2_MODEL_PATH FLUX2_AE_MODEL_PATH QWEN_CACHE_DIR ACTION_INIT
